            ...
            checks.forEach(function(check){
                ...
                _data.read('checks', check, function(err, originalCheckData){
                    if(!err && originalCheckData){
                        // Pass it to the check validator, and let that function continue or log errors needed
...
workers.performCheck = function(originalCheckData){
    ...
    // Pass the hostname and the path out of the orginal check data
    var parsedUrl = url.parse(originalCheckData.protocol + '://' + originalCheckData.url, true);
    var hostName = parsedUrl.hostname;
    var path = parsedUrl.path; // Using path and not "pathname" because we want the string
    ...
    req.on('error', function(e){
        // Update the checkOutcome and passs the data along 
        checkOutcome.error = {
            'error' : true,
            'value' : e
        };
...
// Process the check outcome, update the check data as needed, trigger an alert to user if neede
...
workers.log = function(originalCheckData, checkOutcome, state, alertWarranted, timeOfCheck){
 ...
 // Timer to execute the log log-rotation process once per day
