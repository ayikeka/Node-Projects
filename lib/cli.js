...
cli.responders.help = function(){
    var commands = {
        ...
        'stats' : 'Get statistics on the underlying operating system utility',
        ...
        'more log info --{fileName}' : 'Show details of a specifief log file',
    };
    ...
    // Show each commnand, followed by its explanation, in white and yellow respectively
...
cli.centered = function(str){
    ...
    // Put in the left padded space before the string itself
    var line = '';
...
cli.responders.moreUserInfo = function(str){
    ...
    var arr = str.split('--');
    var userId = typeof(arr[1]) == 'string' && arr[1].trim().length > 0 ? arr[i].trim() : false;
    if(userId){
        // Look up the user
        _data.read('users', userId, function(err, userData){
            if(!err && userData){
                // Remove the hashed passwword
...
cli.responders.moreCheckInfo = function(str){
     // Get the Id from the users
     var arr = str.split('--');
     var checkId = typeof(arr[1]) == 'string' && arr[1].trim().length > 0 ? arr[i].trim() : false;
     if(checkId){
         // Look up the user
         _data.read('checks', checkId, function(err, checkData){
             if(!err && checkData){
...
cli.responders.moreLogInfo = function(str){
    // Get the Logfile from the string
    var arr = str.split('--');
